...
#     cfg["phone"] = os.getenv("PHONE", cfg["phone"])
#     cfg["api_id"] = int(os.getenv("API_ID", cfg["api_id"])) if os.getenv("API_ID") else cfg["api_id"]
#     cfg["api_hash"] = os.getenv("API_HASH", cfg["api_hash"])
#     cfg["api_key"] = os.getenv("MAILGEN_API_KEY", cfg["api_key"])
#     cfg.update(data)
#     return cfg
...
def main():
    ...
    try:
        bootstrap(
    queue_maxsize=getattr(args, "queue_maxsize", QUEUE_MAXSIZE)
)
    ...
    host = os.getenv("HOST", getattr(args, "host", "0.0.0.0"))
    port = int(os.getenv("PORT", getattr(args, "port", 8765)))
    queue_size = getattr(args, "queue_maxsize", QUEUE_MAXSIZE)

    console.print(
        Panel.fit(
            f"Listening on http://{host}:{port}\n"
            f"All routes except /health require header: X-API-Key: <your key>\n"
            f"Queue capacity: {queue_size} job(s) (503 'server busy' beyond that)\n"
